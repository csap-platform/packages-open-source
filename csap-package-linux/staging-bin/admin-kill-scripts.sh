...
function killPattern() {
	...
	ps -ef | grep $killFilter | grep -v grep| grep -v scriptRunAsRoot | grep -v admin-kill-scripts | grep -v DcsapProcessId

	# first kill any scripts issued from UI, running under ssadmin
	processList=`ps -ef | grep $killFilter | grep -v grep| grep -v scriptRunAsRoot | grep -v admin-kill-scripts| grep -v DcsapProcessId`

	if [ "$processList" != "" ] ; then

		printIt "Killing ssadmin $STAGING/scripts"

		#svcPid=`ps -ef | grep /staging/bin | grep -v grep | grep -v admin-kill-scripts | awk '{ print $2 }'`
		parentPid=`ps -ef | grep $killFilter | grep -v grep | grep -v scriptRunAsRoot | grep -v admin-kill-scripts | grep -v DcsapProcessId | awk '{ print $2 }'`
