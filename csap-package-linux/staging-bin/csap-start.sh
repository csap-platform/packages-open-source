...
if [ ! -e "$csapPackageFolder/$serviceName.war" ] \
	&& [   ! -e "$csapPackageFolder/$serviceName.zip" ] \
	&& [   ! -e "$csapPackageFolder/$serviceName.jar" ] ; then
	
	 printIt "Did not find $csapPackageFolder/$serviceName., build must be done"
...
if [ "$serverRuntime" == "wrapper" ] ; then
	...
	printIt "Found a wrapper...extracting $csapPackageFolder/$serviceName.zip to $runDir"
