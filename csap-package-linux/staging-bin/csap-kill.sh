...
if [ "$serviceName" == "CsAgent" ] && [ $isSpawn == "0" ] ; then
	...
	printIt == look for content in $PROCESSING/CsAgentKillNohup.txt file. Wait a few minutes and reload the browser.
	 ...
	 nohup $scriptDir/csap-kill.sh $args -spawn >$PROCESSING/CsAgentKillNohup.txt &
...
showIfDebug  "csap-kill.sh\t:"  processList matching  $serviceName is $processList



showIfDebug  "csap-kill.sh :" parent Pids are $parentPid
...
showIfDebug  "csap-kill.sh\t:" child Pids are $svcPid

# 

if [ "$csapServer" == "wrapper" ] ; then
	showIfDebug  "csap-kill.sh\t:  == Found a wrapper..."
...
if [ $isSuperClean == "1"  ] ; then
	...
	echo  "csap-kill.sh\t:" parent Pids are $parentPid
...
if [ "$serverRuntime" == "SpringBoot" ] ; then
	showIfDebug  "csap-kill.sh\t:" == Spring Boot...
...
showIfDebug  "csap-kill.sh\t:" echo processes found post kill
showIfDebug  "csap-kill.sh\t:" `ps -u $USER -f| grep $serviceName | grep -v -e grep -e killInstance`
...
if [ "$serviceName" == "CsAgent" ] ; then
	  ...
	  if [ ! -d $PROCESSING ] ; then  mkdir $PROCESSING ; fi ;
			...
			nohup $scriptDir/csap-start.sh $args > $PROCESSING/CsAgentStartNohup.txt 2>&1 &
