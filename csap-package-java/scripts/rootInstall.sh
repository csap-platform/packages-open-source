...
function installJava8 () {
	...
	printIt "Installing Java 8"
		
	if [ "$USER" == "root" ] ; then
		printIt root install: $csapWorkingDir version: $minorVersion
		mkdir -p /opt/java
		cd /opt/java ;
		installPath="/opt/java/jdk1.8.0_$minorVersion"
		...
		source $HOME/.cafEnv
		installPath="$STAGING/../java/jdk1.8.0_$minorVersion" 
		if [ "$INSTALL_DIR" != "" ] ; then 
			...
			cd $INSTALL_DIR ;
			installPath="$INSTALL_DIR/jdk1.8.0_$minorVersion" 
